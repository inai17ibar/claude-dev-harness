...
**指摘があった PR は自動マージを止めます。** `review-findings` ラベルを付け、予約済みの
auto-merge を解除します。人が読んで納得したら**ラベルを外すと自動マージが再開**します
（＝ラベルを外すことが承認の操作になります）。指摘が無ければ何もしないので、
そのまま自動で流れます。`--no-review` でレビュー自体を止められます。

これを効かせるには、リポジトリ側の auto-merge ワークフローがラベルを見る必要があります。
`templates/auto-merge.yml` をそのまま置いてください。
...
2つのラベルを使い分けます。どちらも「人が要る」印ですが、誰が外すかが違います。

| ラベル | 意味 | 外すのは |
|---|---|---|
| `needs-attention` | 機械が進めない（CI失敗・コンフリクト） | harness（直れば自動で外れる） |
| `review-findings` | 自動レビューが指摘を出した | **人間**（読んで納得したら） |

`needs-attention` が付くと理由（落ちたチェック名やコンフリクト）が PR にコメントされます。
ラベルは通知であると同時に「もう知らせた」という記録でもあります。nightly は1日9回走るので、
これが無いと同じ PR にコメントが積み上がります。

閉じた PR に残った `needs-attention` は実行時に掃除します。詰まりが直ると auto-merge が
そのままマージしてしまい、open PR を見て回る解除処理が走らないためです。
`is:open label:needs-attention` が信用できる状態を保ちます。
...
claude-dev-harness/          # このリポジトリ = 唯一の正
  ...
  templates/auto-merge.yml   # 対象リポジトリに置く auto-merge ワークフロー
