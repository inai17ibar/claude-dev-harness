...
# 未マージの PR が既にある Issue は作業済みとみなす。
# --force で明示的に上書きできる。
has_open_pr() {
  local issue=$1 count
  count=$(gh pr list --state open --search "linked:issue-$issue" --json number 2>/dev/null | jq 'length' 2>/dev/null || echo 0)
  [ "${count:-0}" -gt 0 ] && return 0
  # linked 検索が効かないリポジトリ向けのフォールバック: ブランチ名で探す
  count=$(gh pr list --state open --json headRefName 2>/dev/null \
    | jq --arg n "issue-${issue}-" '[.[] | select(.headRefName | startswith("agent/" + $n))] | length' 2>/dev/null || echo 0)
  [ "${count:-0}" -gt 0 ]
}
...
run_one() {
  ...
  if ! $FORCE && has_open_pr "$issue"; then
    harness_log "⏭️  Issue #${issue}: 未マージの PR があるためスキップ (--force で上書き)"
    return 0
  fi
