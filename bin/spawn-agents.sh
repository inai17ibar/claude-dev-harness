...
create_pr() {
  local issue=$1 worktree=$2 issue_title=${3:-}
  ...
  # PR タイトルには Issue のタイトルを入れる。
  # "fix: resolve issue #52" だけだと、一覧で見たときに何の PR か分からず、
  # 中身を開くまで判断できない。
  local pr_title
  if [ -n "$issue_title" ]; then
    pr_title="${issue_title} (#${issue})"
  else
    pr_title="fix: resolve issue #${issue}"
  fi
...
run_one() {
  ...
  $AUTO_PR && create_pr "$issue" "$worktree" "$title"
  return 0
}
